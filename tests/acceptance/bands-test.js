...
test('Create a new band', function(assert) {
  ...
  andThen(function() {
    assert.equal(find('.band-link').lenght, 2, "All band links are rendered");
    assert.equal(find('.band-link:last').text().trim(), 'Long Distance Calling', 'Created band appear at the end of the list');
    asset.equal(find('.nav a.active:contains("Songs")').length, 1, "The Song tab is active");
  });
});
